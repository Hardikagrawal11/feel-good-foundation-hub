...
const BloodDonation = () => {
  const { data: campaigns, isLoading } = useCampaigns("Blood Donation");
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {isLoading ? (
              ...
              campaigns.map((camp, i) => (
                <motion.div key={camp.id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-card rounded-xl p-6 border border-border shadow-warm flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
                  <div className="flex flex-col gap-2">
                    <h3 className="font-heading font-semibold text-foreground">{camp.title}</h3>
                    {camp.date && <span className="flex items-center gap-2 text-sm font-medium text-foreground"><Calendar size={16} className="text-primary" /> {camp.date}</span>}
                    {camp.location && <span className="flex items-center gap-2 text-sm text-muted-foreground"><MapPin size={16} /> {camp.location}</span>}
                    {camp.contact && <span className="flex items-center gap-2 text-sm text-muted-foreground"><Phone size={16} /> {camp.contact}</span>}
              ...
              <div className="text-center py-12 bg-card rounded-xl border border-border">
                ...
                <p className="text-muted-foreground font-medium">No upcoming camps at the moment</p>
                <p className="text-sm text-muted-foreground mt-1">Check back soon for new camp announcements!</p>
              </div>
            )}
          </div>
        </div>
